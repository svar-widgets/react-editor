...
import UserMultiselect from '../custom/kanban/UserMultiselect.jsx';
import PriorityCombo from '../custom/kanban/PriorityCombo.jsx';
import './Kanban.css';

registerEditorItem('color-select', ColorSelect);
registerEditorItem('user-multiselect', UserMultiselect);
...
function Kanban() {
  ...
  const listItems = [
    ...
    },
    {
      ...
      label: 'Color',
      key: 'color',
      colors: ['#33B0B4', '#0096FA', '#F1B941'],
    },
    {
      comp: 'slider',
      key: 'progress',
      labelTemplate: (value) => `Progress ${value ?? 0}%`,
    },
    {
      comp: 'date',
      key: 'start_date',
  ...
  return (
    <>
      <h4 style={{ marginLeft: 20 }}>Imitation of kanban cards</h4>

      <div className="wx-wrYigNKZ tasks">
        {cards.map((data) => (
          <div
            key={data.id}
            className={`wx-wrYigNKZ item${activeCard?.id === data.id ? ' active' : ''}`}
            onClick={() => setActiveCard(data)}
          >
            <div className="wx-wrYigNKZ content">{data.label}</div>
          </div>
        ))}
      </div>
